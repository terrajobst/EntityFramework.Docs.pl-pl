...
### <a name="the-qualities-of-testable-code"></a><span data-ttu-id="8c9a7-126">Jakość kodu weryfikowalne</span><span class="sxs-lookup"><span data-stu-id="8c9a7-126">The Qualities of Testable Code</span></span>
...
### <a name="the-unit-of-work-pattern"></a><span data-ttu-id="8c9a7-189">Wzorzec jednostki pracy</span><span class="sxs-lookup"><span data-stu-id="8c9a7-189">The Unit of Work Pattern</span></span>
...
-   <span data-ttu-id="8c9a7-311">Utwórz obiekt newEmployee, aby symulować przychodzące dane.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-311">Create a newEmployee object to simulate incoming data.</span></span>
...
<span data-ttu-id="8c9a7-359">Doskonałe wiadomości to kod wewnątrz metody akcji, który pozostaje w pełni weryfikowalne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-359">The great news is the code inside the action method remains fully testable.</span></span> <span data-ttu-id="8c9a7-360">Nie musimy udostępniać żadnych dodatkowych funkcji dla naszych fałszywych, aby obsługiwać operator include.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-360">We don’t need to provide any additional features for our fakes to support the Include operator.</span></span> <span data-ttu-id="8c9a7-361">W przypadku nieprawidłowych wiadomości musimy użyć operatora include wewnątrz kodu, który chciał zachować trwałość ignorujących.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-361">The bad news is we had to use the Include operator inside of the code we wanted to keep persistence ignorant.</span></span> <span data-ttu-id="8c9a7-362">Jest to podstawowy przykład typu kompromisów, które należy oszacować podczas kompilowania kodu weryfikowalne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-362">This is a prime example of the type of tradeoffs you’ll need to evaluate when building testable code.</span></span> <span data-ttu-id="8c9a7-363">Istnieją przypadki, w których konieczne jest poinformowanie o wycieku, poza abstrakcją repozytorium, aby osiągnąć cele wydajności.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-363">There are times when you need to let persistence concerns leak outside the repository abstraction to meet performance goals.</span></span>

<span data-ttu-id="8c9a7-364">Alternatywą dla ładowania eager jest ładowanie z opóźnieniem.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-364">The alternative to eager loading is lazy loading.</span></span> <span data-ttu-id="8c9a7-365">Ładowanie z opóźnieniem oznacza, że nasz kod firmy *nie* jest potrzebny do jawnego ogłaszania wymagania dotyczącego skojarzonych danych.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-365">Lazy loading means we do *not* need our business code to explicitly announce the requirement for associated data.</span></span> <span data-ttu-id="8c9a7-366">Zamiast tego używamy naszych jednostek w aplikacji, a jeśli potrzebujesz dodatkowych danych Entity Framework załadują dane na żądanie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-366">Instead, we use our entities in the application and if additional data is needed Entity Framework will load the data on demand.</span></span>

### <a name="lazy-loading"></a><span data-ttu-id="8c9a7-367">Ładowanie z opóźnieniem</span><span class="sxs-lookup"><span data-stu-id="8c9a7-367">Lazy Loading</span></span>

<span data-ttu-id="8c9a7-368">Można łatwo przystąpić do scenariusza, w którym nie wiemy, jakie dane będą potrzebne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-368">It’s easy to imagine a scenario where we don’t know what data a piece of business logic will need.</span></span> <span data-ttu-id="8c9a7-369">Firma Microsoft może wiedzieć, że logika wymaga obiektu Employee, ale może odgałęziać się do różnych ścieżek wykonywania, w których niektóre z tych ścieżek wymagają informacji o karcie czasowej od pracownika, a niektóre nie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-369">We might know the logic needs an employee object, but we may branch into different execution paths where some of those paths require time card information from the employee, and some do not.</span></span> <span data-ttu-id="8c9a7-370">Takie scenariusze są idealnym rozwiązaniem dla niejawnego ładowania z opóźnieniem, ponieważ dane są wyświetlane w sposób niezbędny.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-370">Scenarios like this are perfect for implicit lazy loading because data magically appears on an as-needed basis.</span></span>

<span data-ttu-id="8c9a7-371">Ładowanie z opóźnieniem, znane także jako odroczone ładowanie, nakłada pewne wymagania dotyczące obiektów Entity.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-371">Lazy loading, also known as deferred loading, does place some requirements on our entity objects.</span></span> <span data-ttu-id="8c9a7-372">POCOs z prawdziwym ignorujących trwałości nie wpłynie na żadne wymagania z warstwy trwałości, ale prawdziwe ignorujących trwałości jest praktycznie niemożliwe.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-372">POCOs with true persistence ignorance would not face any requirements from the persistence layer, but true persistence ignorance is practically impossible to achieve.</span></span><span data-ttu-id="8c9a7-373">  Zamiast tego mierzę trwałość ignorujących w względnych stopniach.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-373">  Instead we measure persistence ignorance in relative degrees.</span></span> <span data-ttu-id="8c9a7-374">Jeśli konieczne jest dziedziczenie z klasy podstawowej zorientowanej na trwałość lub użycie wyspecjalizowanej kolekcji w celu osiągnięcia ładowania z opóźnieniem w POCOs, może to potrwać.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-374">It would be unfortunate if we needed to inherit from a persistence oriented base class or use a specialized collection to achieve lazy loading in POCOs.</span></span> <span data-ttu-id="8c9a7-375">Na szczęście EF4 ma mniej niepożądane rozwiązanie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-375">Fortunately, EF4 has a less intrusive solution.</span></span>

### <a name="virtually-undetectable"></a><span data-ttu-id="8c9a7-376">Praktycznie niewykrywalny</span><span class="sxs-lookup"><span data-stu-id="8c9a7-376">Virtually Undetectable</span></span>

<span data-ttu-id="8c9a7-377">W przypadku korzystania z obiektów POCO EF4 może dynamicznie generować serwery proxy środowiska uruchomieniowego dla jednostek.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-377">When using POCO objects, EF4 can dynamically generate runtime proxies for entities.</span></span> <span data-ttu-id="8c9a7-378">Te serwery proxy w niewidoczny sposób zawijają POCOs z materiałami i oferują dodatkowe usługi, przechwytuje każdą właściwość pobieranie i ustawianie w celu wykonywania dodatkowych czynności.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-378">These proxies invisibly wrap the materialized POCOs and provide additional services by intercepting each property get and set operation to perform additional work.</span></span> <span data-ttu-id="8c9a7-379">Jedną z tych usług jest funkcja ładowania z opóźnieniem, którą szukamy.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-379">One such service is the lazy loading feature we are looking for.</span></span> <span data-ttu-id="8c9a7-380">Inna usługa to wydajny mechanizm śledzenia zmian, który można rejestrować, gdy program zmienia wartości właściwości jednostki.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-380">Another service is an efficient change tracking mechanism which can record when the program changes the property values of an entity.</span></span> <span data-ttu-id="8c9a7-381">Lista zmian jest używana przez obiekt ObjectContext w metodzie metody SaveChanges, aby zachować wszelkie zmodyfikowane jednostki za pomocą poleceń UPDATE.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-381">The list of changes is used by the ObjectContext during the SaveChanges method to persist any modified entities using UPDATE commands.</span></span>

<span data-ttu-id="8c9a7-382">Jednak aby te serwery proxy działały, muszą one mieć możliwość podłączania do właściwości operacji get i Set w jednostce, a serwery proxy osiągają ten cel przez zastępowanie wirtualnych elementów członkowskich.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-382">For these proxies to work, however, they need a way to hook into property get and set operations on an entity, and the proxies achieve this goal by overriding virtual members.</span></span> <span data-ttu-id="8c9a7-383">Z tego względu, jeśli chcemy niejawnie załadować z opóźnieniem i wydajnym śledzeniem zmian, musimy wrócić do naszych definicji klas POCO i oznaczyć właściwości jako wirtualne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-383">Thus, if we want to have implicit lazy loading and efficient change tracking we need to go back to our POCO class definitions and mark properties as virtual.</span></span>
...
<span data-ttu-id="8c9a7-384">Nadal możemy powiedzieć, że jednostka Employee jest w większości ignorujących trwałość.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-384">We can still say the Employee entity is mostly persistence ignorant.</span></span> <span data-ttu-id="8c9a7-385">Jedynym wymaganiem jest użycie wirtualnych elementów członkowskich. nie ma to wpływu na testowanie kodu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-385">The only requirement is to use virtual members and this does not impact the testability of the code.</span></span> <span data-ttu-id="8c9a7-386">Nie musimy dziedziczyć z żadnej specjalnej klasy bazowej, a nawet użyć specjalnej kolekcji przeznaczonej do ładowania z opóźnieniem.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-386">We don’t need to derive from any special base class, or even use a special collection dedicated to lazy loading.</span></span> <span data-ttu-id="8c9a7-387">Jak ilustruje kod, każda klasa implementująca interfejs ICollection @ no__t-0T @ no__t-1 jest dostępna do przechowywania powiązanych jednostek.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-387">As the code demonstrates, any class implementing ICollection&lt;T&gt; is available to hold related entities.</span></span>

<span data-ttu-id="8c9a7-388">Istnieje również jedna niewielka zmiana, którą trzeba wprowadzić w naszej jednostce pracy.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-388">There is also one minor change we need to make inside our unit of work.</span></span> <span data-ttu-id="8c9a7-389">Ładowanie z opóźnieniem jest domyślnie *wyłączone* podczas pracy bezpośrednio z obiektem ObjectContext.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-389">Lazy loading is *off* by default when working directly with an ObjectContext object.</span></span> <span data-ttu-id="8c9a7-390">Istnieje właściwość, którą można ustawić we właściwości ContextOptions, aby umożliwić ładowanie odroczone i można ustawić tę właściwość wewnątrz rzeczywistej jednostki pracy, jeśli chcemy włączyć ładowanie z opóźnieniem wszędzie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-390">There is a property we can set on the ContextOptions property to enable deferred loading, and we can set this property inside our real unit of work if we want to enable lazy loading everywhere.</span></span>
...
<span data-ttu-id="8c9a7-392">Ładowanie z opóźnieniem sprawia, że kod aplikacji jest łatwiejszy do zapisu, a przy użyciu Magic proxy kod pozostanie całkowicie weryfikowalne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-392">Lazy loading makes the application code easier to write, and with the proxy magic the code remains completely testable.</span></span> <span data-ttu-id="8c9a7-393">W pamięci podręcznej jednostki pracy można po prostu wstępnie załadować fałszywe jednostki ze skojarzonymi danymi, jeśli są one odpowiednie podczas testu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-393">In-memory fakes of the unit of work can simply preload fake entities with associated data when needed during a test.</span></span>

<span data-ttu-id="8c9a7-394">W tym momencie będziemy zachęcać do kompilowania repozytoriów przy użyciu IObjectSet @ no__t-0T @ no__t-1 i przyjrzeć się abstrakcjom w celu ukrycia wszystkich oznak struktury trwałości.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-394">At this point we’ll turn our attention from building repositories using IObjectSet&lt;T&gt; and look at abstractions to hide all signs of the persistence framework.</span></span>

## <a name="custom-repositories"></a><span data-ttu-id="8c9a7-395">Repozytoria niestandardowe</span><span class="sxs-lookup"><span data-stu-id="8c9a7-395">Custom Repositories</span></span>

<span data-ttu-id="8c9a7-396">Po pierwszej prezentowaniu wzorca projektowego jednostki pracy w tym artykule podano przykładowy kod, który może wyglądać jak jednostka pracy.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-396">When we first presented the unit of work design pattern in this article we provided some sample code for what the unit of work might look like.</span></span> <span data-ttu-id="8c9a7-397">Zacznijmy od tego oryginalnego pomysłu przy użyciu scenariusza pracownika i karty czasu pracownika, z którym pracujemy.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-397">Let’s re-present this original idea using the employee and employee time card scenario we’ve been working with.</span></span>
...
<span data-ttu-id="8c9a7-398">Podstawowa różnica między tą jednostką pracy i jednostką pracy utworzoną w ostatniej sekcji polega na tym, jak ta jednostka pracy nie korzysta z żadnych streszczeń z EF4 Framework (nie ma IObjectSet @ no__t-0T @ no__t-1).</span><span class="sxs-lookup"><span data-stu-id="8c9a7-398">The primary difference between this unit of work and the unit of work we created in the last section is how this unit of work does not use any abstractions from the EF4 framework (there is no IObjectSet&lt;T&gt;).</span></span> <span data-ttu-id="8c9a7-399">IObjectSet @ no__t-0T @ no__t-1 działa dobrze jako interfejs repozytorium, ale interfejs API, który uwidacznia, może nie być idealnie wyrównany do potrzeb aplikacji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-399">IObjectSet&lt;T&gt; works well as a repository interface, but the API it exposes might not perfectly align with our application’s needs.</span></span> <span data-ttu-id="8c9a7-400">W tym nadchodzącym podejściu będziemy reprezentować repozytoria przy użyciu niestandardowego IRepository @ no__t-0T @ no__t-1.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-400">In this upcoming approach we will represent repositories using a custom IRepository&lt;T&gt; abstraction.</span></span>

<span data-ttu-id="8c9a7-401">Wielu deweloperów, którzy przestrzegają projektu opartego na testach, projektu opartego na zadziałach i metodologii opartej na warunkach domeny, preferują podejście IRepository @ no__t-0T @ no__t-1 z kilku powodów.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-401">Many developers who follow test-driven design, behavior-driven design, and domain driven methodologies design prefer the IRepository&lt;T&gt; approach for several reasons.</span></span> <span data-ttu-id="8c9a7-402">Najpierw interfejs IRepository @ no__t-0T @ no__t-1 reprezentuje warstwę "Ochrona przed uszkodzeniem".</span><span class="sxs-lookup"><span data-stu-id="8c9a7-402">First, the IRepository&lt;T&gt; interface represents an “anti-corruption” layer.</span></span> <span data-ttu-id="8c9a7-403">Zgodnie z opisem przez Eric Evans w swojej organizacji projektowej opartej na domenie, warstwa antywirusowa utrzymuje kod domeny z interfejsów API infrastruktury, takich jak trwałość interfejsu API.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-403">As described by Eric Evans in his Domain Driven Design book an anti-corruption layer keeps your domain code away from infrastructure APIs, like a persistence API.</span></span> <span data-ttu-id="8c9a7-404">Na koniec deweloperzy mogą tworzyć metody w repozytorium, które spełniają dokładne potrzeby aplikacji (jak zostało to wykryte podczas pisania testów).</span><span class="sxs-lookup"><span data-stu-id="8c9a7-404">Secondly, developers can build methods into the repository that meet the exact needs of an application (as discovered while writing tests).</span></span> <span data-ttu-id="8c9a7-405">Przykładowo może być często konieczne znalezienie pojedynczej jednostki przy użyciu wartości identyfikatora, aby można było dodać metodę FindById do interfejsu repozytorium.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-405">For example, we might frequently need to locate a single entity using an ID value, so we can add a FindById method to the repository interface.</span></span><span data-ttu-id="8c9a7-406">  Nasze IRepository @ no__t-0T @ no__t-1 będą wyglądać następująco.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-406">  Our IRepository&lt;T&gt; definition will look like the following.</span></span>
...
<span data-ttu-id="8c9a7-407">Zwróć uwagę, że będziemy wrócić do korzystania z interfejsu IQueryable @ no__t-0T @ no__t-1 w celu udostępnienia kolekcji jednostek.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-407">Notice we’ll drop back to using an IQueryable&lt;T&gt; interface to expose entity collections.</span></span> <span data-ttu-id="8c9a7-408">Interfejs IQueryable @ no__t-0T @ no__t-1 zezwala na przepływowanie drzew wyrażeń LINQ do dostawcy EF4 i nadawanie dostawcy całościowego widoku zapytania.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-408">IQueryable&lt;T&gt; allows LINQ expression trees to flow into the EF4 provider and give the provider a holistic view of the query.</span></span> <span data-ttu-id="8c9a7-409">Druga opcja zwróci wartość IEnumerable @ no__t-0T @ no__t-1, co oznacza, że dostawca EF4 LINQ zobaczy tylko wyrażenia wbudowane w repozytorium.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-409">A second option would be to return IEnumerable&lt;T&gt;, which means the EF4 LINQ provider will only see the expressions built inside of the repository.</span></span> <span data-ttu-id="8c9a7-410">Wszystkie grupowanie, porządkowanie i projekcje wykonywane poza repozytorium nie zostaną złożone do polecenia SQL wysłanego do bazy danych, co może obniżyć wydajność.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-410">Any grouping, ordering, and projection done outside of the repository will not be composed into the SQL command sent to the database, which can hurt performance.</span></span> <span data-ttu-id="8c9a7-411">Z drugiej strony repozytorium zwracające tylko wartość IEnumerable @ no__t-0T @ no__t-1 spowoduje, że nowe polecenie SQL nie zostanie nigdy zaskakujące.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-411">On the other hand, a repository returning only IEnumerable&lt;T&gt; results will never surprise you with a new SQL command.</span></span> <span data-ttu-id="8c9a7-412">Obie metody będą działać, a oba podejścia pozostaną weryfikowalne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-412">Both approaches will work, and both approaches remain testable.</span></span>

<span data-ttu-id="8c9a7-413">Jest to proste, aby zapewnić jedną implementację interfejsu IRepository @ no__t-0T @ no__t-1 przy użyciu typów ogólnych i interfejsu API programu EF4 ObjectContext.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-413">It’s straightforward to provide a single implementation of the IRepository&lt;T&gt; interface using generics and the EF4 ObjectContext API.</span></span>
...
<span data-ttu-id="8c9a7-414">IRepository @ no__t-0T @ no__t-1 zapewnia nam dodatkową kontrolę nad naszymi zapytaniami, ponieważ klient musi wywołać metodę, aby uzyskać dostęp do jednostki.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-414">The IRepository&lt;T&gt; approach gives us some additional control over our queries because a client has to invoke a method to get to an entity.</span></span> <span data-ttu-id="8c9a7-415">Wewnątrz metody możemy udostępnić dodatkowe sprawdzenia i operatory LINQ w celu wymuszenia ograniczeń aplikacji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-415">Inside the method we could provide additional checks and LINQ operators to enforce application constraints.</span></span> <span data-ttu-id="8c9a7-416">Zauważ, że interfejs ma dwa ograniczenia dla parametru typu ogólnego.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-416">Notice the interface has two constraints on the generic type parameter.</span></span> <span data-ttu-id="8c9a7-417">Pierwsze ograniczenie to wady klas wymagane przez obiekt ObjectSet @ no__t-0T @ no__t-1, a drugie ograniczenie wymusza, aby nasze jednostki implementują IEntity — abstrakcję utworzoną dla aplikacji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-417">The first constraint is the class cons taint required by ObjectSet&lt;T&gt;, and the second constraint forces our entities to implement IEntity – an abstraction created for the application.</span></span> <span data-ttu-id="8c9a7-418">Interfejs IEntity wymusza, aby jednostki miały Właściwość identyfikatora z możliwością odczytu, a następnie można użyć tej właściwości w metodzie FindById.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-418">The IEntity interface forces entities to have a readable Id property, and we can then use this property in the FindById method.</span></span> <span data-ttu-id="8c9a7-419">IEntity jest zdefiniowany przy użyciu następującego kodu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-419">IEntity is defined with the following code.</span></span>
...
<span data-ttu-id="8c9a7-420">IEntity może być traktowany jako niewielkie naruszenie trwałości ignorujących, ponieważ nasze jednostki są wymagane do zaimplementowania tego interfejsu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-420">IEntity could be considered a small violation of persistence ignorance since our entities are required to implement this interface.</span></span> <span data-ttu-id="8c9a7-421">Pamiętaj, że trwałość ignorujących ma wpływ na kompromisy, a w przypadku wielu funkcji FindById będzie przewyższał ograniczenie narzucone przez interfejs.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-421">Remember persistence ignorance is about tradeoffs, and for many the FindById functionality will outweigh the constraint imposed by the interface.</span></span> <span data-ttu-id="8c9a7-422">Interfejs nie ma wpływu na testowanie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-422">The interface has no impact on testability.</span></span>

<span data-ttu-id="8c9a7-423">Utworzenie wystąpienia elementu Live IRepository @ no__t-0T @ no__t-1 wymaga elementu EF4 ObjectContext, więc konkretną jednostką implementacji pracy powinna zarządzać Tworzenie wystąpienia.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-423">Instantiating a live IRepository&lt;T&gt; requires an EF4 ObjectContext, so a concrete unit of work implementation should manage the instantiation.</span></span>
...
### <a name="using-the-custom-repository"></a><span data-ttu-id="8c9a7-424">Korzystanie z repozytorium niestandardowego</span><span class="sxs-lookup"><span data-stu-id="8c9a7-424">Using the Custom Repository</span></span>

<span data-ttu-id="8c9a7-425">Korzystanie z naszego niestandardowego repozytorium nie różni się znacznie od użycia repozytorium w oparciu o IObjectSet @ no__t-0T @ no__t-1.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-425">Using our custom repository is not significantly different from using the repository based on IObjectSet&lt;T&gt;.</span></span> <span data-ttu-id="8c9a7-426">Zamiast stosować operatory LINQ bezpośrednio do właściwości, najpierw musimy wywoływać metody tego repozytorium, aby uzyskać informacje o interfejsie IQueryable @ no__t-0T @ no__t-1.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-426">Instead of applying LINQ operators directly to a property, we’ll first need to invoke one the repository’s methods to grab an IQueryable&lt;T&gt; reference.</span></span>
...
<span data-ttu-id="8c9a7-427">Zwróć uwagę, że niestandardowy operator dołączania będzie działał bez zmian.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-427">Notice the custom Include operator we implemented previously will work without change.</span></span> <span data-ttu-id="8c9a7-428">Metoda FindById repozytorium usuwa zduplikowaną logikę z akcji próbujących pobrać pojedynczą jednostkę.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-428">The repository’s FindById method removes duplicated logic from actions trying to retrieve a single entity.</span></span>
...
<span data-ttu-id="8c9a7-429">Nie ma znaczącej różnicy w zakresie testowania dwóch rozważanych metod.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-429">There is no significant difference in the testability of the two approaches we’ve examined.</span></span> <span data-ttu-id="8c9a7-430">Możemy zapewnić fałszywe implementacje IRepository @ no__t-0T @ no__t-1 przez utworzenie konkretnych klas, których kopie zapasowe są wykonywane przez HashSet — @ no__t-2Employee @ no__t-3 — podobnie jak w ostatniej sekcji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-430">We could provide fake implementations of IRepository&lt;T&gt; by building concrete classes backed by HashSet&lt;Employee&gt; - just like what we did in the last section.</span></span> <span data-ttu-id="8c9a7-431">Jednak niektórzy deweloperzy wolą używać obiektów tworzenia i makietowania struktur obiektów zamiast tworzyć sztuczne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-431">However, some developers prefer to use mock objects and mock object frameworks instead of building fakes.</span></span> <span data-ttu-id="8c9a7-432">Zobaczmy, jak używać makietów do testowania implementacji i omówienia różnic między fragmentami i elementami sztucznymi w następnej sekcji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-432">We’ll look at using mocks to test our implementation and discuss the differences between mocks and fakes in the next section.</span></span>

### <a name="testing-with-mocks"></a><span data-ttu-id="8c9a7-433">Testowanie za pomocą makiet</span><span class="sxs-lookup"><span data-stu-id="8c9a7-433">Testing with Mocks</span></span>

<span data-ttu-id="8c9a7-434">Istnieją różne podejścia do kompilowania, które Fowlera dzwoni "test Double".</span><span class="sxs-lookup"><span data-stu-id="8c9a7-434">There are different approaches to building what Martin Fowler calls a “test double”.</span></span> <span data-ttu-id="8c9a7-435">Test Double (taki jak stunt Movie Double) jest obiektem, który kompiluje się do "w rzeczywistości" dla rzeczywistych obiektów produkcyjnych podczas testów.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-435">A test double (like a movie stunt double) is an object you build to “stand in” for real, production objects during tests.</span></span> <span data-ttu-id="8c9a7-436">Utworzone repozytoria w pamięci są testami podwójne dla repozytoriów, które komunikują się SQL Server.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-436">The in-memory repositories we created are test doubles for the repositories that talk to SQL Server.</span></span> <span data-ttu-id="8c9a7-437">Dowiesz się, jak używać tych testów podczas testów jednostkowych w celu odizolowania kodu i zapewnienia szybkiego uruchamiania testów.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-437">We’ve seen how to use these test-doubles during the unit tests to isolate code and keep tests running fast.</span></span>

<span data-ttu-id="8c9a7-438">Testy zostały skompilowane z rzeczywistymi, działającymi implementacjami.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-438">The test doubles we’ve built have real, working implementations.</span></span> <span data-ttu-id="8c9a7-439">W tle każdy z nich przechowuje konkretną kolekcję obiektów i dodaje i usuwa obiekty z tej kolekcji podczas manipulowania repozytorium podczas testu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-439">Behind the scenes each one stores a concrete collection of objects, and they will add and remove objects from this collection as we manipulate the repository during a test.</span></span> <span data-ttu-id="8c9a7-440">Niektórzy deweloperzy, którzy lubią kompilację testu, w ten sposób podwajają się w ten sposób, korzystając z prawdziwych i wydajnych implementacji</span><span class="sxs-lookup"><span data-stu-id="8c9a7-440">Some developers like to build their test doubles this way – with real code and working implementations.</span></span><span data-ttu-id="8c9a7-441">  Ten test podwaja te elementy, które wywołujemy *fałszywe*.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-441">  These test doubles are what we call *fakes*.</span></span> <span data-ttu-id="8c9a7-442">Mają one pracę z implementacjami, ale nie są one wystarczające do użycia w środowisku produkcyjnym.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-442">They have working implementations, but they aren’t real enough for production use.</span></span> <span data-ttu-id="8c9a7-443">Fałszywe repozytorium nie zapisuje w bazie danych.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-443">The fake repository doesn’t actually write to the database.</span></span> <span data-ttu-id="8c9a7-444">Fałszywy serwer SMTP nie wysyła w rzeczywistości wiadomości e-mail za pośrednictwem sieci.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-444">The fake SMTP server doesn’t actually send an email message over the network.</span></span>

### <a name="mocks-versus-fakes"></a><span data-ttu-id="8c9a7-445">Elementy w przeciwieństwie do elementów sztucznych</span><span class="sxs-lookup"><span data-stu-id="8c9a7-445">Mocks versus Fakes</span></span>

<span data-ttu-id="8c9a7-446">Istnieje inny typ testu podwójnie znany jako *makieta*.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-446">There is another type of test double known as a *mock*.</span></span> <span data-ttu-id="8c9a7-447">Podczas gdy elementy sztuczne mają działające implementacje, makiety nie są implementowane.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-447">While fakes have working implementations, mocks come with no implementation.</span></span> <span data-ttu-id="8c9a7-448">Dzięki pomocy dotyczącej struktury obiektów makiety tworzymy te obiekty w czasie wykonywania i używają ich jako podwójnego przetestowania.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-448">With the help of a mock object framework we construct these mock objects at run time and use them as test doubles.</span></span> <span data-ttu-id="8c9a7-449">W tej sekcji będziemy korzystać z struktury "Moqing" "open source".</span><span class="sxs-lookup"><span data-stu-id="8c9a7-449">In this section we’ll be using the open source mocking framework Moq.</span></span> <span data-ttu-id="8c9a7-450">Oto prosty przykład użycia MOQ do dynamicznego tworzenia testów dla repozytorium pracowników.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-450">Here is a simple example of using Moq to dynamically create a test double for an employee repository.</span></span>
...
<span data-ttu-id="8c9a7-451">Zaproponujemy MOQ dla implementacji IRepository @ no__t-0Employee @ no__t-1 i tworzy ją dynamicznie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-451">We ask Moq for an IRepository&lt;Employee&gt; implementation and it builds one dynamically.</span></span> <span data-ttu-id="8c9a7-452">Możemy uzyskać do obiektu implementującego IRepository @ no__t-0Employee @ no__t-1, uzyskując dostęp do właściwości Object obiektu DB@ no__t-2T @ no__t-3.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-452">We can get to the object implementing IRepository&lt;Employee&gt; by accessing the Object property of the Mock&lt;T&gt; object.</span></span> <span data-ttu-id="8c9a7-453">Jest to ten obiekt wewnętrzny, który można przekazać do naszych kontrolerów i nie będzie wiadomo, czy jest to test podwójny, czy rzeczywiste repozytorium.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-453">It is this inner object we can pass into our controllers, and they won’t know if this is a test double or the real repository.</span></span> <span data-ttu-id="8c9a7-454">Możemy wywoływać metody na obiekcie tak samo, jak możemy wywołać metody dla obiektu z rzeczywistą implementacją.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-454">We can invoke methods on the object just like we would invoke methods on an object with a real implementation.</span></span>

<span data-ttu-id="8c9a7-455">Należy zastanawiać się, co będzie miało repozytorium makiety po wywołaniu metody Add.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-455">You must be wondering what the mock repository will do when we invoke the Add method.</span></span> <span data-ttu-id="8c9a7-456">Ponieważ nie istnieje implementacja za obiektem makiety, nic nie robi.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-456">Since there is no implementation behind the mock object, Add does nothing.</span></span> <span data-ttu-id="8c9a7-457">Nie ma żadnej konkretnej kolekcji w tle, podobnie jak w przypadku zapisana przez nas sfałszowanych, więc pracownik jest odrzucany.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-457">There is no concrete collection behind the scenes like we had with the fakes we wrote, so the employee is discarded.</span></span> <span data-ttu-id="8c9a7-458">Co z wartością zwracaną z FindById?</span><span class="sxs-lookup"><span data-stu-id="8c9a7-458">What about the return value of FindById?</span></span> <span data-ttu-id="8c9a7-459">W takim przypadku obiekt makiety robi tylko to, co może zrobić, co spowoduje zwrócenie wartości domyślnej.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-459">In this case the mock object does the only thing it can do, which is return a default value.</span></span> <span data-ttu-id="8c9a7-460">Ponieważ zwracamy typ referencyjny (pracownika), wartość zwracana jest wartością null.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-460">Since we are returning a reference type (an Employee), the return value is a null value.</span></span>

<span data-ttu-id="8c9a7-461">Makiety mogą dźwiękować bezwartościowe; Istnieją jednak dwie inne funkcje makiet, o których nie podano informacji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-461">Mocks might sound worthless; however, there are two more features of mocks we haven’t talked about.</span></span> <span data-ttu-id="8c9a7-462">Najpierw platforma MOQ rejestruje wszystkie wywołania wykonane na obiekcie makiety.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-462">First, the Moq framework records all the calls made on the mock object.</span></span> <span data-ttu-id="8c9a7-463">Później można polecić MOQ, jeśli ktoś wywołał metodę Add, lub jeśli ktoś wywołał metodę FindById.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-463">Later in the code we can ask Moq if anyone invoked the Add method, or if anyone invoked the FindById method.</span></span> <span data-ttu-id="8c9a7-464">Zobaczymy później, w jaki sposób możemy użyć tej funkcji nagrywania "czarnego pola" w testach.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-464">We’ll see later how we can use this “black box” recording feature in tests.</span></span>

<span data-ttu-id="8c9a7-465">Druga świetna funkcja polega na tym, jak możemy używać MOQ do programowania obiektu makiety z *oczekiwaniami*.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-465">The second great feature is how we can use Moq to program a mock object with *expectations*.</span></span> <span data-ttu-id="8c9a7-466">Oczekiwanie nakazuje obiektowi makiety, jak odpowiedzieć na daną interakcję.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-466">An expectation tells the mock object how to respond to any given interaction.</span></span> <span data-ttu-id="8c9a7-467">Na przykład możemy zaprogramować oczekiwanie w naszym makietie i poinstruować go, aby zwracał obiekt Employee, gdy ktoś wywoła FindById.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-467">For example, we can program an expectation into our mock and tell it to return an employee object when someone invokes FindById.</span></span> <span data-ttu-id="8c9a7-468">Struktura MOQ używa interfejsu API Instalatora i wyrażeń lambda do zaprogramowania tych oczekiwań.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-468">The Moq framework uses a Setup API and lambda expressions to program these expectations.</span></span>
...
<span data-ttu-id="8c9a7-469">W tym przykładzie poprosił MOQ o dynamiczne skompilowanie repozytorium, a następnie programuje repozytorium z oczekiwaniami.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-469">In this sample we ask Moq to dynamically build a repository, and then we program the repository with an expectation.</span></span> <span data-ttu-id="8c9a7-470">Oczekiwanie nakazuje obiektowi imitacji zwrócenie nowego obiektu pracownika z wartością identyfikatora 5, gdy ktoś wywoła metodę FindById, przekazując wartość 5.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-470">The expectation tells the mock object to return a new employee object with an Id value of 5 when someone invokes the FindById method passing a value of 5.</span></span> <span data-ttu-id="8c9a7-471">Ten test kończy się niepowodzeniem i nie musimy kompilować pełnej implementacji dla fałszywych IRepository @ no__t-0T @ no__t-1.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-471">This test passes, and we didn’t need to build a full implementation to fake IRepository&lt;T&gt;.</span></span>

<span data-ttu-id="8c9a7-472">Ponownie odwiedzamy wcześniej wykonane testy i przeprowadzimy je do korzystania z form zamiast fałszywych.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-472">Let’s revisit the tests we wrote earlier and rework them to use mocks instead of fakes.</span></span> <span data-ttu-id="8c9a7-473">Podobnie jak wcześniej, użyjemy klasy bazowej do skonfigurowania wspólnych części infrastruktury potrzebnej dla wszystkich testów kontrolera.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-473">Just like before, we’ll use a base class to setup the common pieces of infrastructure we need for all of the controller’s tests.</span></span>
...
<span data-ttu-id="8c9a7-474">Kod instalatora pozostaje w większości tego samego.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-474">The setup code remains mostly the same.</span></span> <span data-ttu-id="8c9a7-475">Zamiast używać fałszywych, będziemy używać MOQ do konstruowania obiektów makiety.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-475">Instead of using fakes, we’ll use Moq to construct mock objects.</span></span> <span data-ttu-id="8c9a7-476">Klasa bazowa jest rozmieszczenia dla jednostki, która będzie zwracać repozytorium, gdy kod wywołuje właściwość Employees.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-476">The base class arranges for the mock unit of work to return a mock repository when code invokes the Employees property.</span></span> <span data-ttu-id="8c9a7-477">Pozostała część konfiguracji makiety będzie odbywać się w ramach armatury testowej przeznaczonych dla każdego konkretnego scenariusza.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-477">The rest of the mock setup will take place inside the test fixtures dedicated to each specific scenario.</span></span> <span data-ttu-id="8c9a7-478">Na przykład, armatura testowa dla akcji indeks spowoduje skonfigurowanie repozytorium makiety, aby zwracało listę pracowników, gdy akcja wywoła metodę FindAll repozytorium.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-478">For example, the test fixture for the Index action will setup the mock repository to return a list of employees when the action invokes the FindAll method of the mock repository.</span></span>
...
<span data-ttu-id="8c9a7-479">Z wyjątkiem oczekiwań, nasze testy wyglądają podobnie jak testy, które wcześniej istniały.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-479">Except for the expectations, our tests look similar to the tests we had before.</span></span> <span data-ttu-id="8c9a7-480">Jednak dzięki możliwości rejestrowania struktury makiety możemy dochodzić do testowania z innego kąta.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-480">However, with the recording ability of a mock framework we can approach testing from a different angle.</span></span> <span data-ttu-id="8c9a7-481">Ta nowa perspektywa zostanie wyświetlona w następnej sekcji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-481">We’ll look at this new perspective in the next section.</span></span>

### <a name="state-versus-interaction-testing"></a><span data-ttu-id="8c9a7-482">Stan a testowanie interakcji</span><span class="sxs-lookup"><span data-stu-id="8c9a7-482">State versus Interaction Testing</span></span>

<span data-ttu-id="8c9a7-483">Istnieją różne techniki, których można użyć do testowania oprogramowania z obiektami makiety.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-483">There are different techniques you can use to test software with mock objects.</span></span> <span data-ttu-id="8c9a7-484">Jednym z metod jest użycie testowania opartego na stanie, co jest gotowe do wykonania w tym dokumencie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-484">One approach is to use state based testing, which is what we have done in this paper so far.</span></span> <span data-ttu-id="8c9a7-485">Testowanie na podstawie stanu pozwala na potwierdzenie stanu oprogramowania.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-485">State based testing makes assertions about the state of the software.</span></span> <span data-ttu-id="8c9a7-486">W ostatnim teście wywołana została metoda działania na kontrolerze i złożyła potwierdzenie dotyczące modelu, który powinien zostać skompilowany.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-486">In the last test we invoked an action method on the controller and made an assertion about the model it should build.</span></span> <span data-ttu-id="8c9a7-487">Oto kilka innych przykładów stanu testowania:</span><span class="sxs-lookup"><span data-stu-id="8c9a7-487">Here are some other examples of testing state:</span></span>

-   <span data-ttu-id="8c9a7-488">Sprawdź, czy repozytorium zawiera nowy obiekt Employee po wykonaniu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-488">Verify the repository contains the new employee object after Create executes.</span></span>
-   <span data-ttu-id="8c9a7-489">Sprawdź, czy model zawiera listę wszystkich pracowników po wykonaniu indeksu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-489">Verify the model holds a list of all employees after Index executes.</span></span>
-   <span data-ttu-id="8c9a7-490">Upewnij się, że repozytorium nie zawiera danego pracownika po wykonaniu usuwania.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-490">Verify the repository does not contain a given employee after Delete executes.</span></span>

<span data-ttu-id="8c9a7-491">Inne podejście, które zobaczysz, jest widoczne z obiektami makiety, aby zweryfikować *interakcje*.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-491">Another approach you’ll see with mock objects is to verify *interactions*.</span></span> <span data-ttu-id="8c9a7-492">Podczas testowania opartego na stanie są sprawdzane informacje o stanie obiektów, testowanie na podstawie interakcji sprawia, że obiekty są w interakcji z interakcją.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-492">While state based testing makes assertions about the state of objects, interaction based testing makes assertions about how objects interact.</span></span> <span data-ttu-id="8c9a7-493">Na przykład:</span><span class="sxs-lookup"><span data-stu-id="8c9a7-493">For example:</span></span>

-   <span data-ttu-id="8c9a7-494">Upewnij się, że kontroler wywołuje metodę Add repozytorium podczas wykonywania tworzenia.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-494">Verify the controller invokes the repository’s Add method when Create executes.</span></span>
-   <span data-ttu-id="8c9a7-495">Sprawdź, czy kontroler wywołuje metodę FindAll repozytorium, gdy jest wykonywane indeksowanie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-495">Verify the controller invokes the repository’s FindAll method when Index executes.</span></span>
-   <span data-ttu-id="8c9a7-496">Sprawdź, czy kontroler wywołuje metodę zatwierdzania jednostki pracy, aby zapisać zmiany po wykonaniu edycji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-496">Verify the controller invokes the unit of work’s Commit method to save changes when Edit executes.</span></span>

<span data-ttu-id="8c9a7-497">Testowanie interakcji często wymaga mniej danych testowych, ponieważ nie są one poking wewnątrz kolekcji i nie sprawdzają liczby.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-497">Interaction testing often requires less test data, because we aren’t poking inside of collections and verifying counts.</span></span> <span data-ttu-id="8c9a7-498">Na przykład, jeśli wiemy, że akcja Details wywołuje metodę FindById repozytorium z poprawną wartością, to działanie prawdopodobnie działa poprawnie.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-498">For example, if we know the Details action invokes a repository’s FindById method with the correct value - then the action is probably behaving correctly.</span></span> <span data-ttu-id="8c9a7-499">Możemy sprawdzić to zachowanie bez konfigurowania jakichkolwiek danych testowych do zwrócenia z FindById.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-499">We can verify this behavior without setting up any test data to return from FindById.</span></span>
...
<span data-ttu-id="8c9a7-500">Jedyną konfiguracją wymaganą w powyższym zasobie testowym jest instalacja dostarczana przez klasę bazową.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-500">The only setup required in the above test fixture is the setup provided by the base class.</span></span> <span data-ttu-id="8c9a7-501">Gdy wywołamy akcję kontrolera, MOQ będzie rejestrować interakcje z repozytorium makiety.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-501">When we invoke the controller action, Moq will record the interactions with the mock repository.</span></span> <span data-ttu-id="8c9a7-502">Za pomocą weryfikowania interfejsu API MOQ można polecić MOQ, Jeśli kontroler wywołał FindById z prawidłową wartością identyfikatora.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-502">Using the Verify API of Moq, we can ask Moq if the controller invoked FindById with the proper ID value.</span></span> <span data-ttu-id="8c9a7-503">Jeśli kontroler nie wywołał metody lub wywołaniu metody z nieoczekiwaną wartością parametru, Metoda verify zgłosi wyjątek, a test zakończy się niepowodzeniem.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-503">If the controller did not invoke the method, or invoked the method with an unexpected parameter value, the Verify method will throw an exception and the test will fail.</span></span>

<span data-ttu-id="8c9a7-504">Oto inny przykład, aby sprawdzić, czy akcja tworzenia wywołuje zatwierdzenie w bieżącej jednostce pracy.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-504">Here is another example to verify the Create action invokes Commit on the current unit of work.</span></span>
...
<span data-ttu-id="8c9a7-505">Jedno zagrożenie z testowaniem interakcji to tendencja do określania interakcji.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-505">One danger with interaction testing is the tendency to over specify interactions.</span></span> <span data-ttu-id="8c9a7-506">Możliwość rejestrowania i weryfikowania każdej interakcji z obiektem makiety nie oznacza, że test powinien weryfikować każdą interakcję.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-506">The ability of the mock object to record and verify every interaction with the mock object doesn’t mean the test should try to verify every interaction.</span></span> <span data-ttu-id="8c9a7-507">Niektóre interakcje są szczegółami implementacji i weryfikują interakcje *wymagane* do spełnienia bieżącego testu.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-507">Some interactions are implementation details and you should only verify the interactions *required* to satisfy the current test.</span></span>

<span data-ttu-id="8c9a7-508">Wybór między makietami lub sztucznymi jest w dużym stopniu zależny od testowanego systemu i preferencji osobistych (lub zespołów).</span><span class="sxs-lookup"><span data-stu-id="8c9a7-508">The choice between mocks or fakes largely depends on the system you are testing and your personal (or team) preferences.</span></span> <span data-ttu-id="8c9a7-509">Obiekty makiety mogą znacząco zmniejszyć ilość kodu wymaganego do wdrożenia testów, ale nie każdy z nich jest wygodny dla oczekiwań programistycznych i sprawdza interakcje.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-509">Mock objects can drastically reduce the amount of code you need to implement test doubles, but not everyone is comfortable programming expectations and verifying interactions.</span></span>

## <a name="conclusions"></a><span data-ttu-id="8c9a7-510">Wnioski</span><span class="sxs-lookup"><span data-stu-id="8c9a7-510">Conclusions</span></span>

<span data-ttu-id="8c9a7-511">W tym dokumencie przedstawiono kilka metod tworzenia kodu weryfikowalne przy użyciu Entity Framework ADO.NET w celu zapewnienia trwałości danych.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-511">In this paper we’ve demonstrated several approaches to creating testable code while using the ADO.NET Entity Framework for data persistence.</span></span> <span data-ttu-id="8c9a7-512">Możemy wykorzystać skompilowane streszczenia, takie jak IObjectSet @ no__t-0T @ no__t-1 lub utworzyć własne streszczenia, takie jak IRepository @ no__t-2T @ no__t-3.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-512">We can leverage built in abstractions like IObjectSet&lt;T&gt;, or create our own abstractions like IRepository&lt;T&gt;.</span></span><span data-ttu-id="8c9a7-513">  W obu przypadkach wsparcie POCO w ADO.NET Entity Framework 4,0 umożliwia konsumentom tych streszczeń pozostawanie trwałych ignorujących i wysoce weryfikowalne.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-513">  In both cases, the POCO support in the ADO.NET Entity Framework 4.0 allows the consumers of these abstractions to remain persistent ignorant and highly testable.</span></span> <span data-ttu-id="8c9a7-514">Dodatkowe funkcje EF4, takie jak niejawne ładowanie z opóźnieniem, umożliwiają działanie kodu usługi biznesowej i aplikacji bez konieczności pojmowania się szczegółami relacyjnego magazynu danych.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-514">Additional EF4 features like implicit lazy loading allows business and application service code to work without worrying about the details of a relational data store.</span></span> <span data-ttu-id="8c9a7-515">Na koniec, tworzone streszczenia są łatwe do zawinięcia lub fałszywe wewnątrz testów jednostkowych, a firma Microsoft może używać tych testów w celu szybkiego uruchamiania, wysoce odizolowanych i niezawodnych testów.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-515">Finally, the abstractions we create are easy to mock or fake inside of unit tests, and we can use these test doubles to achieve fast running, highly isolated, and reliable tests.</span></span>

### <a name="additional-resources"></a><span data-ttu-id="8c9a7-516">Dodatkowe zasoby</span><span class="sxs-lookup"><span data-stu-id="8c9a7-516">Additional Resources</span></span>

-   <span data-ttu-id="8c9a7-517">Robert C. Martin, " [Pojedyncza zasada odpowiedzialności](https://www.objectmentor.com/resources/articles/srp.pdf)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-517">Robert C. Martin, “ [The Single Responsibility Principle](https://www.objectmentor.com/resources/articles/srp.pdf)”</span></span>
-   <span data-ttu-id="8c9a7-518">Fowlera Martin, [Katalog wzorców](https://www.martinfowler.com/eaaCatalog/index.html) ze *wzorców architektury aplikacji dla przedsiębiorstw*</span><span class="sxs-lookup"><span data-stu-id="8c9a7-518">Martin Fowler, [Catalog of Patterns](https://www.martinfowler.com/eaaCatalog/index.html) from *Patterns of Enterprise Application Architecture*</span></span>
-   <span data-ttu-id="8c9a7-519">Griffin Caprio, " [iniekcja zależności](https://msdn.microsoft.com/magazine/cc163739.aspx)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-519">Griffin Caprio, “ [Dependency Injection](https://msdn.microsoft.com/magazine/cc163739.aspx)”</span></span>
-   <span data-ttu-id="8c9a7-520">Blog dotyczący programowania danych "@no__t 0Walkthrough: Programowanie sterowane testami za pomocą Entity Framework 4.0 @ no__t-0 ".</span><span class="sxs-lookup"><span data-stu-id="8c9a7-520">Data Programmability Blog, “ [Walkthrough: Test Driven Development with the Entity Framework 4.0](https://blogs.msdn.com/adonet/pages/walkthrough-test-driven-development-with-the-entity-framework-4-0.aspx)”.</span></span>
-   <span data-ttu-id="8c9a7-521">Blog dotyczący programowania danych, " [używanie wzorców repozytorium i jednostki pracy z Entity Framework 4,0](https://blogs.msdn.com/adonet/archive/2009/06/16/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0.aspx)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-521">Data Programmability Blog, “ [Using Repository and Unit of Work patterns with Entity Framework 4.0](https://blogs.msdn.com/adonet/archive/2009/06/16/using-repository-and-unit-of-work-patterns-with-entity-framework-4-0.aspx)”</span></span>
-   <span data-ttu-id="8c9a7-522">Aaron Jensen, " [Wprowadzenie specyfikacji maszyn](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-522">Aaron Jensen, “ [Introducing Machine Specifications](http://codebetter.com/blogs/aaron.jensen/archive/2008/05/08/introducing-machine-specifications-or-mspec-for-short.aspx)”</span></span>
-   <span data-ttu-id="8c9a7-523">Eric Lewandowski, " [BDD with MSTest](https://blogs.msdn.com/elee/archive/2009/01/20/bdd-with-mstest.aspx)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-523">Eric Lee, “ [BDD with MSTest](https://blogs.msdn.com/elee/archive/2009/01/20/bdd-with-mstest.aspx)”</span></span>
-   <span data-ttu-id="8c9a7-524">Eric Evans, " [Projektowanie oparte na domenie](https://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-524">Eric Evans, “ [Domain Driven Design](https://books.google.com/books?id=7dlaMs0SECsC&printsec=frontcover&dq=evans%20domain%20driven%20design&hl=en&ei=cHztS6C8KIaglAfA_dS1CA&sa=X&oi=book_result&ct=result&resnum=1&ved=0CCoQ6AEwAA)”</span></span>
-   <span data-ttu-id="8c9a7-525">Fowlera Martin, " [imitacje nie są fragmentami](https://martinfowler.com/articles/mocksArentStubs.html)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-525">Martin Fowler, “ [Mocks Aren’t Stubs](https://martinfowler.com/articles/mocksArentStubs.html)”</span></span>
-   <span data-ttu-id="8c9a7-526">Fowlera Martin, " [test Double](https://martinfowler.com/bliki/TestDouble.html)"</span><span class="sxs-lookup"><span data-stu-id="8c9a7-526">Martin Fowler, “ [Test Double](https://martinfowler.com/bliki/TestDouble.html)”</span></span>
-   [<span data-ttu-id="8c9a7-527">Moq</span><span class="sxs-lookup"><span data-stu-id="8c9a7-527">Moq</span></span>](https://code.google.com/p/moq/)

### <a name="biography"></a><span data-ttu-id="8c9a7-528">Biografii</span><span class="sxs-lookup"><span data-stu-id="8c9a7-528">Biography</span></span>

<span data-ttu-id="8c9a7-529">Scott, który jest członkiem działu technicznego w Pluralsight i założyciel OdeToCode.com.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-529">Scott Allen is a member of the technical staff at Pluralsight and the founder of OdeToCode.com.</span></span> <span data-ttu-id="8c9a7-530">W trakcie komercyjnego programowania oprogramowania Scott pracował nad rozwiązaniami dla wszystkich urządzeń z 8-bitowymi urządzeniami osadzonymi w wysoce skalowalnych aplikacjach sieci Web ASP.NET.</span><span class="sxs-lookup"><span data-stu-id="8c9a7-530">In 15 years of commercial software development, Scott has worked on solutions for everything from 8-bit embedded devices to highly scalable ASP.NET web applications.</span></span> <span data-ttu-id="8c9a7-531">Możesz skontaktować się z Scott w swoim blogu w witrynie OdeToCode lub w serwisie Twitter w [https://twitter.com/OdeToCode](https://twitter.com/OdeToCode).</span><span class="sxs-lookup"><span data-stu-id="8c9a7-531">You can reach Scott on his blog at OdeToCode, or on Twitter at [https://twitter.com/OdeToCode](https://twitter.com/OdeToCode).</span></span>
