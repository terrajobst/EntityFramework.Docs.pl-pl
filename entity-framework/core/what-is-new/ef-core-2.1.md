...
## <a name="data-seeding"></a>Rozmieszczanie danych
...
- Obsługa jako zwracany typ dla `FromSql()` zapytań
...
Teraz można określić właściwości nawigacji zdefiniowane tylko w typach pochodnych podczas pisania wyrażeń dla `Include` metody. W przypadku silnie wpisanej `Include`wersji, firma Microsoft obsługuje używanie jawnego rzutowania `as` lub operatora. Teraz obsługujemy również odwoływanie się do nazw właściwości nawigacji zdefiniowanej w typach pochodnych w wersji `Include`ciągu:
...
Zwróć uwagę, że to zapytanie zostanie przetłumaczone tylko na dwie zapytania SQL: Jeden dla klientów i następnych dla zamówień.
...
Nowe `Tracked` i `StateChanged` zdarzenia dotyczące`ChangeTracker` programu mogą służyć do pisania logiki, która reaguje na jednostki wchodzące w obiekt DbContext lub zmieniając ich stan.

## <a name="raw-sql-parameter-analyzer"></a>Nieprzetworzony Analizator parametrów SQL

Do EF Core jest dołączany nowy analizator kodu, który wykrywa potencjalnie niebezpieczne użycia interfejsów API RAW-SQL, takich `FromSql` jak `ExecuteSqlCommand`lub. Na przykład dla poniższego zapytania zobaczysz ostrzeżenie, ponieważ _minAge_ nie jest sparametryzowane:
