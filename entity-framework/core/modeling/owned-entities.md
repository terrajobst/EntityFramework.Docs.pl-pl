...
Należy do jednostki, które typy nigdy nie znajdują się w podstawowej EF w modelu przez Konwencję. Można użyć `OwnsOne` metody w `OnModelCreating` lub Dodaj adnotację do typu z `OwnedAttrbibute` (nowy 2.1 Core EF) do konfigurowania typu jako typu należące do firmy.
